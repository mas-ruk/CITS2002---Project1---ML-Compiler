...
// Boolean function to check if a function name already exists
bool FunctionAlreadyExists(const char* identifier) {
    for (int i = 0; i < FunctionsCount; i++) {
        if (strcmp(ExistingFunctions[i], identifier) == 0) {
            return true;  // function identifier found in array
        }
    }
    return false;  // function identifier not found in array
}
...
// Expecting open bracket
            if (pCurrentTkn().type == TknLBracket) {
                ...
                else if {pCurrentTkn().type == TknIdentifier) { // means existance of variables used by the function
                    //do function thingys
...
//find identifier at start of line
void pAssnOrFuncCall() {
    char funcNameBuffer[100];
    strcpy(funcNameBuffer, pCurrentTkn().value);
    if (FunctionAlreadyExists(funcNameBuffer)) {
        pFuncCall();
    }
    else {
        pAssignment();
...
// Check if the function name already exists using the boolean function
if (isFunctionAlreadyDefined(functionName)) {
    printf("Error: Function '%s' is already defined!\n", functionName);
    exit(1);  // Handle error, function name already exists
} else {
    addFunctionName(functionName);  // Add new function name to the list
}
